...
**Metadata is a shared, capped budget, not a per-skill allowance.** In budget mode, the visibility hook renders the complete wrapped skills block under `visibility_token_budget` (default **2500** tokens): wrapper, both section headers, every skill name, and model-invocable routing detail all compete for the same budget. Every effective visible skill remains listed at least by name; `disable-model-invocation: true` skills are name-only under `Manual skills (load by name; /name when user-invocable):`. The heading is literal: only `user-invocable: true` makes `/name` available; otherwise use exact-name loading. If the complete name-only floor exceeds the configured budget, the renderer retains every name and adds a deterministic overflow notice directing the agent to `load_skill(list=true)` or `load_skill(search="…")`; it does not pretend the cap held. If only `max_skills_visible` is configured, the legacy count-cap renderer remains in effect: it alphabetically limits model-invocable entries and renders user-invoked entries separately with their condensed descriptions. A verbose description competes with all other model-invocable detail in budget mode, so it can push them down detail tiers.
